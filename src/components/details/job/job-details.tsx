"use client"
import { createApplication, getApplicationsByStudent } from "@/api/application-api"
...
import type { EmployerInterface } from "@/interfaces/user-interface"
import { useUserStore } from "@/stores/user-store"
import { ApplicationValidationSchema } from "@/validations/application-validation"
import { Form, Formik } from "formik"
import { StarIcon, MapPin, Clock, Banknote, Building, Save, Filter, PersonStanding, Link2 } from "lucide-react"
            ...
            <div className="flex justify-center">
              <Dialog>
                ...
                <DialogContent className="max-w-2xl">
                  <DialogHeader>
                    <DialogTitle className="text-lg font-semibold mb-4">Chọn CV của bạn để ứng tuyển</DialogTitle>
                  </DialogHeader>
                  <Formik
                      ...
                      <Form onSubmit={handleSubmit}>
                        <div className="space-y-4">
                          ...
                          >
                            {userResumes.length > 0 ? (
                              userResumes.map((resume) => (
                                ...
                                >
                                  <div className="flex items-center space-x-3">
                                    <RadioGroupItem value={resume.resumeID ?? ""} id={`resume-${resume.resumeID}`} />
                                    <Label
                                      htmlFor={`resume-${resume.resumeID}`}
                                      className="text-sm font-medium cursor-pointer"
                                    >
                                      {resume.jobTitle || resume.fullName || "CV không có tiêu đề"}
                                    </Label>
                                  </div>
                                  <Link
                              ...
                              <div className="text-center py-8">
                                <div className="text-gray-500 mb-2">Bạn chưa có CV nào.</div>
                                <Link href="/cv/add" className="text-blue-600 hover:underline text-sm">
                                  Tạo CV mới
                                </Link>
                              </div>
                            )}
                          ...
                          {errors.resumeID && touched.resumeID && (
                            <div className="text-red-600 text-sm mt-2">{errors.resumeID}</div>
                          )}
                        </div>
                        ...
                        {errors.coverletter && touched.coverletter && (
                          <div className="text-red-600 text-sm">{errors.coverletter}</div>
                        )}
                        <DialogFooter>
          ...
          <div className="lg:col-span-2">
            <Card className="shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" onClick={() => alert("Chia sẻ công việc")}>
              ...
              <CardContent>
                <div className="prose max-w-none">
                  ...
                  <h3 className="text-lg font-semibold mb-4 mt-8">Yêu cầu công việc</h3>
